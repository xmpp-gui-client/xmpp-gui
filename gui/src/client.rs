...
impl State{
    ...
    async fn conn(&mut self) {
        let mut client_builder = ClientBuilder::new(&self.jid,&self.passwd).set_client(ClientType::Pc, "xmpp-rs");
    ...
    pub fn connect(&mut self){
        ...
        // call async connect method
        // let res = timeout(Duration::from_secs(1), self.conn()).await;
        self.conn();
        // ();
    }
...
fn test_connect_failure_disconnect() {
    ...
    state.connect();
    assert_eq!(state.status.to_lowercase(),"disconnected");
}
